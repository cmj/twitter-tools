...
# Grab guest token
#guest_token="$(((($(date +%s) * 1000) -1288834974657) << 22))${RANDOM:0:3}"
...
# sso-init (removing for recent 403)
#"${curl_}" -sv "https://api.x.com/1.1/onboarding/sso_init.json" "${header[@]}" -d "{\"provider\": \"apple\"}" -o /dev/null

# start flow ("check_logged_in_account":1 -> 2)
...
# sso_init again? (removing for recent 403)
#curl -s "https://api.x.com/1.1/onboarding/sso_init.json" "${header[@]}" -d "{\"provider\": \"apple\"}" #-o /dev/null
...
output="-o /dev/null"
[[ "$debug" -eq 1 ]] && output="" && echo -e "\e[0;32m#### AccountDuplicationCheck\e[0m"
"${curl_}" -s "${output}" "${base_url}" "${header[@]}" -H "X-Csrf-Token: ${csrf}" \
