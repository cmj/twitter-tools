...
# and create cookie.json for other tools, twikit, etc.
# [requires Twitter account] 

# Doesn't work 100% of the time, takes a few tries: 
# for ((i=1;i<=6;i++)); do if twitter-cookie.sh; exit 0; fi; sleep 5; done
...
if [ "$denied_check" ]; then echo -e "\e[31m$denied_check\e[0m"; exit 1; fi
...
echo "xCsrfToken = \"${ct0}\""
echo "--- cookies.json file saved (https://github.com/d60/twikit/issues/227) ---"
