#!/bin/bash
# Fetch user subscriptions via Twitter handle
...
# .data.user.result.has_hidden_subscriptions_on_profile
output=$(user-id | jq -r '.data.user.result | "Subscriptions: \(.creator_subscriptions_count)  Hidden: \(.has_hidden_subscriptions_on_profile)"')
echo $output
grep -q 'Subscriptions: 0 ' <<< "${output}" && exit 1
user_id=$(user-id | jq -r .data.user.result.rest_id)

api=https://x.com/i/api/graphql/LE6RjmjkSMWorQcJu55wFg/UserCreatorSubscriptions
variables='{"userId":"'"${user_id}"'","limit":100,"includePromotedContent":false,"withBirdwatchNotes":true,"withVoice":true}'
...
curl -s -G "${header[@]}" $api \
  --data-urlencode "variables=${variables}" \
  --data-urlencode "features=${features}" |
  jq -r 'if(.data.user.result.timeline.timeline.instructions[-1]) then .data.user.result.timeline.timeline.instructions[-1].entries[].content.itemContent.user_results.result.legacy.screen_name | select( . != null ) else "No subscriptions" end'
